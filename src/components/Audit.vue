              ...
              <div class="mb-md-5 mt-md-2 pb-5">
                ...
                <div class="form-outline form-white mb-4">
                  <table class="table">
                    ...
                    <tbody>
                      <tr v-for="item in audit.students" :key="item.id">
                        <td>{{ item.id }}</td>
                        <td>{{ item.previousName }}</td>
                        <td>{{ item.newName }}</td>
                        <td>{{ item.previousProgram }}</td>
                        <td>{{ item.newProgram }}</td>
                        <td>{{ item.modify }}</td>
                        <td>{{ item.process }}</td>
                        <td>{{ item.dbUser }}</td>
...
export default {
  ...
  setup() {
    ...
    const audit = reactive({ students: [] })
    const goBack = () => router.push('Reports')

    onMounted(() => {
      Axios.get('/audits')
        .then((resp) => audit.students = resp.data)
